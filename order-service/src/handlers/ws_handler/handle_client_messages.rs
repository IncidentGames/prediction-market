...
pub(super) async fn handle_client_messages(state: &Arc<AppState>, message: &ClientMessage) {
            ...
            if let Some(channel_type) = channel {
                match channel_type {
                    ChannelType::OrderBookUpdate(market_id) => {
                        log_info!(
                            "Subscribing to order book updates for market: {}",
                            market_id
                        );
        ...
        MessagePayload::Unsubscribe { channel } => {
            ...
            if let Some(channel_type) = channel {
                match channel_type {
                    ChannelType::OrderBookUpdate(market_id) => {
                        log_info!(
                            "Unsubscribing from order book updates for market: {}",
                            market_id
                        );
                        let mut market_subs = state.market_subs.write();
                    ...
                    _ => {
                        log_warn!(
                            "Unsupported channel type for unsubscription: {:?}",
                            channel_type
                        );
                    }
                }
